...
public class LabelledNode{
	...
	private LabelledNode lastNode;
	private String name;
	private String tree;
	private String state = "<empty state>";
	private String parentState;
	private String [] availableStates;
	private ArrayList<LabelledNode> daughters;
...
//c		// default no-arg constructor
	...
	public LabelledNode(ArrayList<LabelledNode> input, int current_global_node_count) throws HardPolytomyException{
//c		// internal node constructor
		this.global_node_count = current_global_node_count +1;
		this.node_id = global_node_count;
		daughters = input;
		if(daughters.size()>2){
//			TODO commented this out 2009 09 29
//			throw new HardPolytomyException();
		}	
	...
	public LabelledNode(ArrayList<String> taxaNames, ArrayList<String> taxaStates, String tree, int lastPos, String[] availableStatesIn) throws HardPolytomyException{
...
// end of addition
		name = "topnode";
		distance = 0;
		daughters = new ArrayList<LabelledNode>();
		int treeLength = tree.length();
		int i = lastPos;
		boolean end = false;
		LabelledNode lastNode = new LabelledNode();
//o		state = "<empty state>";
		isContested = true;
...
//-				System.out.println(tree + "\n");
				LabelledNode n = new LabelledNode(tree, i, taxaNames, taxaStates, availableStates, availableIntStates, global_node_count);
				lastNode = n;
				global_node_count = n.getGlobal_node_count();
				daughters.add(n);
...
//c				// set lastnode to this
				...
				int newPos = Integer.parseInt(nodeNameAndNewPos.get(1));
				i = newPos;
				LabelledNode n = new LabelledNode(newPos, nodeName, taxaNames, taxaStates, availableStates, availableIntStates, global_node_count);
				lastNode = n;
				global_node_count = n.getGlobal_node_count();
				daughters.add(n);
			}
 ...
 */		
		if(daughters.size()>2){
//			TODO commented this out.... 29092009
//			throw new HardPolytomyException();
		}	
		if(daughters.size()>2){
...
//0		this.finalFitch();
	...
	public LabelledNode(int lastPos, String input, ArrayList<String> taxaNames, ArrayList<String> taxaStates, String[] availableStates, int[] availableIntStates, int current_global_node_count){
...
//-		System.out.println("   called external constructor");
		...
		this.global_node_count = current_global_node_count;
		this.node_id = 0;
		this.taxaNames = taxaNames;
		this.name = input;
...
//-		printStateWeights();
	...
	public LabelledNode(String tree, int lastPos, ArrayList<String> taxaNames, ArrayList<String> taxaStates, String[] availableStates, int[] availableIntStates, int current_global_node_count) throws HardPolytomyException{
...
//-				System.out.println("\t\t=====INTERNAL CALL=====");
				if(doubleStep){
					LabelledNode n = new LabelledNode(tree, lastPos - 1, taxaNames, taxaStates, availableStates, availableIntStates, global_node_count);  // rev 4.0
					global_node_count = n.getGlobal_node_count();
					lastNode = n;
				} else {
					LabelledNode n = new LabelledNode(tree, lastPos, taxaNames, taxaStates, availableStates, availableIntStates, global_node_count);  // rev 3.2 here -> tree..
					global_node_count = n.getGlobal_node_count();
					lastNode = n;
				}
...
//o					lastPos = newPos;
					i = newPos;
					LabelledNode n = new LabelledNode(i, nodeName, taxaNames, taxaStates, availableStates, availableIntStates, global_node_count);
					lastNode = n;
					global_node_count = n.getGlobal_node_count();
					daughters.add(n);
					lastNodeExists = true;
...
//-		System.out.println();
		...
		for(LabelledNode kid:daughters){
 ...
 */
		if(daughters.size()>2){
//			TODO commented this out 29 09 2009
//			throw new HardPolytomyException();
		}	
...
//c			// terminal
			...
			for(LabelledNode thisnode:daughters){
 ...
 */		
		if(!isTerminal){
			for(LabelledNode child:daughters){
/*
...
//		add to states any states in parent not in this one
	...
	//-					System.out.println(ai);
						for (LabelledNode daughter : daughters) {
	...
	public float[] getAllStats() {
		...
		float ai = 0f;
		float UF = 0f;
		float[] PDs = new float[availableStates.length];
		float[] MCs = new float[availableStates.length];
		float[] retArr = new float[7+(availableStates.length*2)];
		for(float val:retArr){
			val = 0f;
		}
		allSize = this.distance;
		intSize = this.distance;

		if(isTop&&2>3){
			System.out.println("TOPPPP");
			AI = 0;
			for (LabelledNode daughter : daughters) {
				float[]daughterStats = daughter.getAllStats();
				allSize += daughterStats[0];
				intSize += daughterStats[1];
				AI += daughterStats[2];
				PS += daughterStats[3];
				UF += daughterStats[4];
			}
...
//-				System.out.println(name + " "+ MaxN);
				if(this.getAstateWeight(state)>0){
					PDs[state] += distance;
					if(this.isMonophyletic()){
						MCs[state] = this.getAstateWeight(state);
					}
				}
			}
			ai = (1 - (MaxN / numDaughters))/(float) (Math.pow(2, (numDaughters - 1)));
//			System.out.println(ai);
			for (LabelledNode daughter : daughters) {
				float[]daughterStats = daughter.getAllStats();
				allSize += daughterStats[0];
				intSize += daughterStats[1];
				sumAI += daughterStats[2];
				PS += daughterStats[3];
				UF += daughterStats[4];
				
				for(int state:availableIntStates){
					PDs[state] += daughterStats[7+state];
					if(daughterStats[7+availableStates.length+state]>MCs[state]){
						MCs[state] = daughterStats[7+availableStates.length+state];
					}
				}
			}
			if(this.isMonophyletic()){
...
//		System.out.println("\t"+name+" AI: ["+AI+"]");
		...
		for(int i=0; i<availableStates.length; i++){
			retArr[7+i] = PDs[i];
			retArr[7+availableStates.length+i] = MCs[i];
		}
		return retArr;
	}
	
	public ArrayList<LabelledNode> getImmediateDaughters(){
		return daughters;
	}
	...
	public float getBestHeight(){
		...
		if(isTerminal){
			...
			for(LabelledNode contender:daughters){
...
*/
	...
	public float[] getUniFrac(){
		...
		if(isTerminal){
			...
			uniFrac = 0f;
			for(LabelledNode aNode:daughters){
 ...
 */
	...
	public int getGlobal_node_count() {
		return global_node_count;
	}

	public void setGlobal_node_count(int global_node_count) {
		this.global_node_count = global_node_count;
	}
...
//-				System.out.println("some unreconstructed nodes");
	...
	public String setNameInternalNodes(){
		if(isTerminal){
		} else {
			if(isTop){
				for(LabelledNode thisnode:daughters){
					String daughterNodeName = thisnode.setNameInternalNodes();
				}
			} else {
				String newName = "internal"+node_id+"[";
				for(LabelledNode thisnode:daughters){
					String daughterNodeName = thisnode.setNameInternalNodes();
...
//-		System.out.println(">------" + item + Position);
	...
	public void setHeight(float newHeight){
		if(isTerminal){
			...
			for(LabelledNode lowerNode:daughters){
...
//	}
	...
	public void simpleShowNodes(){
		if(isTerminal){
			System.out.println(name+"\t"+stateWeights[0] + " " + stateWeights[1] + " " + state + " " + name + " d:" + distance + " h:" + height);
		}else{
			System.out.println(" - ");
			System.out.println(name+"\t"+stateWeights[0] + " " + stateWeights[1] + " " + state + " " + name + " d:" + distance + " h:" + height + taxaGeneralizedFitchStates.isIntersection() + " states: " + taxaGeneralizedFitchStates.getStates());
			for(LabelledNode thisnode:daughters){thisnode.simpleShowNodes();}
		}
	}
	
	public void simplerShowNodes(){
		if(isTop){
			System.out.println(name+"\t"+height);
			for(LabelledNode thisnode:daughters){thisnode.simplerShowNodes();}
		}else if(isTerminal){
			System.out.println("external"+name+"\t"+distance);
		}else{
			System.out.println("internal"+node_id+"\t"+height);
			for(LabelledNode thisnode:daughters){thisnode.simplerShowNodes();}
		}
	}
