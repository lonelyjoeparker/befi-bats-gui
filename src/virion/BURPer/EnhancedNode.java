...
public class EnhancedNode{
	...
	private EnhancedNode lastNode;
	private String name;
	private String tree;
	private String state = "<empty state>";
	private String parentState;
	private String [] availableStates;
	private ArrayList<EnhancedNode> daughters;
...
//c		// default no-arg constructor
	...
	public EnhancedNode(ArrayList<EnhancedNode> input) throws HardPolytomyException{
//c		// internal node constructor
	...
	public EnhancedNode(ArrayList<String> taxaNames, ArrayList<String> taxaStates, String tree, int lastPos, String[] availableStatesIn) throws HardPolytomyException{
...
// end of addition
		name = "topnode";
		distance = 0;
		daughters = new ArrayList<EnhancedNode>();
		int treeLength = tree.length();
		int i = lastPos;
		boolean end = false;
		EnhancedNode lastNode = new EnhancedNode();
//o		state = "<empty state>";
		isContested = true;
...
//-				System.out.println(tree + "\n");
				EnhancedNode n = new EnhancedNode(tree, i, taxaNames, taxaStates, availableStates, availableIntStates);
				lastNode = n;
				daughters.add(n);
...
//c				// set lastnode to this
				...
				int newPos = Integer.parseInt(nodeNameAndNewPos.get(1));
				i = newPos;
				EnhancedNode n = new EnhancedNode(newPos, nodeName, taxaNames, taxaStates, availableStates, availableIntStates);
				lastNode = n;
				daughters.add(n);
			}
			i ++;
...
//0		this.finalFitch();
	...
	public EnhancedNode(int lastPos, String input, ArrayList<String> taxaNames, ArrayList<String> taxaStates, String[] availableStates, int[] availableIntStates){
...
//-		printStateWeights();
	...
	public EnhancedNode(String tree, int lastPos, ArrayList<String> taxaNames, ArrayList<String> taxaStates, String[] availableStates, int[] availableIntStates) throws HardPolytomyException{
...
//-				System.out.println("\t\t=====INTERNAL CALL=====");
				if(doubleStep){
					EnhancedNode n = new EnhancedNode(tree, lastPos - 1, taxaNames, taxaStates, availableStates, availableIntStates);  // rev 4.0
					lastNode = n;
				} else {
					EnhancedNode n = new EnhancedNode(tree, lastPos, taxaNames, taxaStates, availableStates, availableIntStates);  // rev 3.2 here -> tree..
					lastNode = n;
				}
...
//o					lastPos = newPos;
					i = newPos;
					EnhancedNode n = new EnhancedNode(i, nodeName, taxaNames, taxaStates, availableStates, availableIntStates);
					lastNode = n;
					daughters.add(n);
					lastNodeExists = true;
				}
...
//-		System.out.println();
		...
		for(EnhancedNode kid:daughters){
...
//c			// terminal
			...
			for(EnhancedNode thisnode:daughters){
 ...
 */		
		if(!isTerminal){
			for(EnhancedNode child:daughters){
/*
...
//		add to states any states in parent not in this one
	...
	//-					System.out.println(ai);
						for (EnhancedNode daughter : daughters) {
...
//		System.out.println("\t"+name+" AI: ["+AI+"]");
	...
	public ArrayList<EnhancedNode> getImmediateDaughters(){
		return daughters;
	}
	...
	public float getBestHeight(){
		...
		if(isTerminal){
			...
			for(EnhancedNode contender:daughters){
...
*/
	...
	public float[] getUniFrac(){
		...
		if(isTerminal){
			...
			uniFrac = 0f;
			for(EnhancedNode aNode:daughters){
...
//-		System.out.println(">------" + item + Position);
	...
	public void setHeight(float newHeight){
		if(isTerminal){
			...
			for(EnhancedNode lowerNode:daughters){
...
//	}
	...
	public void simpleShowNodes(){
		if(isTerminal){
			...
			for(EnhancedNode thisnode:daughters){thisnode.simpleShowNodes();}
		}
	}
